...
from pywps import Process, FORMATS, ComplexOutput, ComplexInput
from qgis.core import QgsProcessing, QgsProcessingFeedback, QgsProcessingMultiStepFeedback


class ChannelNetworkAndDrainageBasins(Process):
    def __init__(self):
        inputs = [
            ComplexInput('input', 'INP file', supported_formats=[FORMATS.JSON])
        ]
        outputs = [
            ComplexOutput('DIRECTION', 'Buffered file', supported_formats=[FORMATS.JSON]),
            ComplexOutput('CONNECTION', 'Buffered file', supported_formats=[FORMATS.JSON]),
            ComplexOutput('ORDER', 'Buffered file', supported_formats=[FORMATS.JSON]),
            ComplexOutput('BASIN', 'Buffered file', supported_formats=[FORMATS.JSON]),
            ComplexOutput('SEGMENTS', 'Buffered file', supported_formats=[FORMATS.JSON]),
            ComplexOutput('BASINS', 'Buffered file', supported_formats=[FORMATS.JSON]),
            ComplexOutput('NODES', 'Buffered file', supported_formats=[FORMATS.JSON])
        ]
    ...
    def _handler(self, request, response):
        ...
        input_tiff = r'C:\Users\admin\Desktop\PyWPS\ASTGTM2_N30E114\ASTGTM2_N30E114_dem.tif'
        ...
        outputs['ChannelNetworkAndDrainageBasins'] = processing.run('saga:channelnetworkanddrainagebasins',
                                                                    alg_params,
